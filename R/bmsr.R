...
runSTAN <- function(file,data,opts)
{
  ...
  #}
  if (file == 'bmsr.stan')
    model = stanmodels$bmsr
  if (file == 'bmsmtr.stan')
    model = stanmodels$bmsmtr
  ...
  if(opts$inference == "Sampling")
  {
    for(i in 1:length(seeds))
    {
      out = try(rstan::sampling(
        ...
        , iter = sampling_iterations, warmup = sampling_iterations/2, refresh = sampling_iterations/10 #show an update @ each %10
